...
function entity_modifier.reset_player_model(player_name, player)
	...
	if not player_properties then
		core.chat_send_player(player_name, "Nothing to reset for "..player_name)
		return
	end
	...
	player_initial_properties[player_name] = nil
	core.chat_send_player(player_name, "Your player model is reset to the default: "..player_name)
...
function entity_modifier.disguise_player(player, model_name, model_properties)
	...
	if (model_properties.hp_max or 1) < core.PLAYER_MAX_HP_DEFAULT then
		model_properties.hp_max = core.PLAYER_MAX_HP_DEFAULT
	end
	...
	core.chat_send_player(player_name, "Disguised to " .. model_name)
end
...
function entity_modifier.copy_to_or_clear_clipboard(player_name, object_name, properties)
	...
	if copied_model and object_name == copied_model.name then
		player_clipboard[player_name] = nil
		core.chat_send_player(player_name, "Clipboard cleared")
	else
		player_clipboard[player_name] = {name = object_name, properties = properties}
		core.chat_send_player(player_name, "Model copied: "..object_name)
	end
end

function entity_modifier.disguise_tool_primary(_, player, pointed_object)
	local player_name = player:get_player_name()
	if not core.check_player_privs(player, { disguise=true }) then
		core.chat_send_player(player_name, "You have not the `disguise` permission")
		return
	end

	if pointed_object.type == "node" then
		-- Get the definition of the node that was hit
		local node_name = core.get_node(pointed_object.under).name
	...
	elseif pointed_object.type == "object" then
		...
		if not copied_model then
			...
			if not properties then
				core.chat_send_player(player_name, "Not existing entity")
				return
			end
			...
			core.chat_send_player(player_name, "Model copied: "..name)
			return
		end
		...
		if next(copied_model) == nil then
			if pointed_object.ref:is_player() then
				...
				core.chat_send_player(player_name, "Entity model reset: "..name)
			end
			return
		...
		core.chat_send_player(player_name, "Entity "..name.." disguised to "..copied_model.name)
...
function entity_modifier.get_model_properties(player_name, model_name)
	local node = core.registered_nodes[model_name]
	if node then
		return entity_modifier.get_node_disguise(node)
	end

	local model_properties = core.registered_entities[model_name]
	if model_properties then
		return model_properties
	end

	model_properties = core.get_player_by_name(model_name)
	if model_properties then
		return model_properties
	end

	core.chat_send_player(player_name, "Unsupported model name: " .. model_name)
end
...
function entity_modifier.disguise_to_model(player_name, model_name, doer_name)
	...
	if not player then
		core.chat_send_player(doer_name, "Invalid player name: " .. player_name)
	end
	...
	if not model_name or model_name == "" then
		...
		if clipboard_model then
			...
			if doer_name ~= player_name then
				core.chat_send_player(doer_name, "Player model reset")
			end
			return
		end
	end
	...
	if model_properties then
		...
		core.chat_send_player(doer_name, "Player "..player_name.." is disguised to "..model_name)
	end
end
...
local function get_clipboard(name)
	...
	core.chat_send_player(name, "Clipboard is empty")
end

core.register_privilege("clipboard_edit", {
	description = "Can edit clipboard properties"
})

core.register_chatcommand("clipboard", {
	...
	func = function(name, arg)
		...
		elseif first_char == "*" then
			...
			core.chat_send_player(name, "Reset mode enabled")
		elseif first_char == "." then
			local clipboard = get_clipboard(name)
			...
			if found then
				...
				if not core.check_player_privs(name, { clipboard_edit=true }) then
...
core.register_privilege("disguiseme", {
	description = "Can disguise itself"
})

core.register_chatcommand("disguiseme", {
...
core.register_privilege("disguise", {
	description = "Can disguise players"
})

core.register_chatcommand("disguise", {
...
function entity_modifier.disguise_tool_secondary(_, player, _)
	...
	elseif player_initial_properties[player_name] then
		...
		core.chat_send_player(player_name, "Model copied to clipboard: " .. player_name)
	end
end

core.register_tool("entity_modifier:disguise_wand", {
...
core.register_chatcommand("list_models", {
	...
	func = function(name, namespace)
		...
		core.chat_send_player(name, "Available entity model names:" .. model_names)
	end
})
