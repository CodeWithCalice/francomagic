...
local function split_components(items)
	...
	for i, k in pairs(items) do
		...
		index = index + 1
		if index == 4 then
			-- On ajoute pas les recettes vides pour éviter de faire des calculs inutiles
			local blanks = 0
...
local function compare_find(splitup)
	...
	for _,recipe in pairs(magicalities.arcane.recipes) do
		-- Peu importe si ce n'est pas le bon nombre de lignes, on vérifie juste que les lignes présentes correspondent à la recette
			...
			for index, row in pairs(recipe.input) do
				...
				for i, cell in pairs(row) do
					if cell:find("group:") == 1 then
						if core.get_item_group(splitup[index][i], cell:gsub("group:", "")) > 0 then
							cells = cells + 1
						end
...
function magicalities.arcane.get_recipe(items)
	...
	local result = {new_input = {}, output = recipe.output, requirements = recipe.requirements, level_requirement = recipe.level_requirement}
...
local function allow_metadata_inventory_put (pos, listname, index, stack, player)
	...
	if (listname == "wand" and core.get_item_group(stack:get_name(), "wand") == 0) or listname == "craftres" then
		return 0
	end
...
local function update_craft(pos)
	...
	-- On vérifie qu'une baguette est dans la table
	-- On vérifie que le joueur a le niveau de magie requis pour la recette
	local wand = inv:get_stack("wand", 1)
	local level = get_level_witch(magicalities.wands.get_wand_owner(wand))
	if not recipe or wand:is_empty() or (recipe.level_requirement > level) then
...
-- Table des arcanes
core.register_node("magicalities:arcane_table", {
	description = "Table des arcanes",
	tiles = {
		"magicalities_table_arcane_top.png", "magicalities_table_bottom.png", "magicalities_table_arcane.png",
	...
	on_metadata_inventory_take = function(pos, listname, index, stack, player)
		...
		if listname == "craftres" then
			...
			if res then
				...
				-- On récupère les éléments de la baguette nécessaires à la recette
				local wand = inv:get_stack("wand", 1)
...
-- Pupitre
core.register_node("magicalities:table", {
	description = "Table",
	drawtype = "nodebox",
	tiles = {
