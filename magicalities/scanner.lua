...
local function show_spec(i, placer, pointed_thing)
	...
	if not node or core.get_item_group(node.name, "crystal_cluster") == 0 then
		return i
	end
	...
	core.show_formspec(placer:get_player_name(), "magicalities:crystal_scanner", create_formspec(contents, nodedef.description))
	return i
end

core.register_craftitem("magicalities:element_ring", {
